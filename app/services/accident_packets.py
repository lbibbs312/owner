...
def create_accident_report_from_form(form, files, *, user, damage_report=None, driver_log=None, report_context=None):
    now = datetime.utcnow()
    context = report_context or {}
    driver = getattr(damage_report, "reported_by", None) or getattr(driver_log, "driver", None) or user
    anyone_hurt = _clean(form.get("anyone_hurt"))
    other_vehicle_quick = _clean(form.get("other_vehicle_involved_quick"))
    property_damage_quick = _clean(form.get("property_damage_quick"))
    police_called_quick = _clean(form.get("police_called_quick"))
    tow_away_needed = _clean(form.get("tow_away_needed"))
    vehicle_safe_quick = _clean(form.get("vehicle_safe_to_operate_quick"))
    manager_notified_quick = _clean(form.get("manager_notified_quick"))
    police_called = _clean(form.get("police_called")) or police_called_quick
    claim_opened = _clean(form.get("claim_opened"))
    report = AccidentIncidentReport(
        damage_report_id=getattr(damage_report, "id", None),
        driver_log_id=getattr(driver_log, "id", None),
        stop_id=getattr(driver_log, "id", None) or context.get("stop_id"),
        driver_id=getattr(driver, "id", None),
        created_by_id=user.id,
        updated_by_id=user.id,
        incident_date_time=_parse_datetime(form.get("incident_date_time") or context.get("incident_datetime_value")),
        reported_date_time=_parse_datetime(form.get("reported_date_time") or context.get("reported_datetime_value")) or now,
        truck=_clean(form.get("truck")) or getattr(damage_report, "truck_number", None) or context.get("truck"),
        trailer=_clean(form.get("trailer")) or getattr(damage_report, "trailer_number", None) or context.get("trailer"),
        route_id=_clean(form.get("route_id")) or context.get("route_id"),
        plant_or_location=_clean(form.get("plant_or_location")) or getattr(damage_report, "plant_name", None) or getattr(driver_log, "plant_name", None) or context.get("plant_or_location"),
        exact_location_text=_clean(form.get("exact_location_text")) or context.get("exact_location_text"),
        ...
        city=_clean(form.get("city")) or context.get("city"),
        state=_clean(form.get("state")) or context.get("state"),
        ...
        other_vehicle_involved_quick=other_vehicle_quick,
        property_damage_quick=property_damage_quick,
        police_called_quick=police_called_quick,
        tow_away_needed=tow_away_needed,
        vehicle_safe_to_operate_quick=vehicle_safe_quick,
        manager_notified_quick=manager_notified_quick,
        ...
        property_damage=_truthy(form.get("property_damage")) or property_damage_quick == "yes",
        injury_reported=_truthy(form.get("injury_reported")) or anyone_hurt == "yes",
        tow_away=_truthy(form.get("tow_away")) or tow_away_needed == "yes",
        police_called=police_called,
        other_vehicle_involved=_truthy(form.get("other_vehicle_involved")) or other_vehicle_quick == "yes",
        claim_expected=_truthy(form.get("claim_expected")) or claim_opened == "yes",
        ...
        vehicle_safe_to_operate=_clean(form.get("vehicle_safe_to_operate")) or vehicle_safe_quick,
        repair_needed=_clean(form.get("repair_needed")),
        vehicle_removed_from_service=_clean(form.get("vehicle_removed_from_service")),
        ...
        citation_issued=_clean(form.get("citation_issued")),
        citation_to_company_driver=_clean(form.get("citation_to_company_driver")),
        ...
        public_road_in_commerce=_clean(form.get("public_road_in_commerce")),
        fatality=_clean(form.get("fatality")),
        ...
        medical_treatment_away_from_scene=_clean(form.get("medical_treatment_away_from_scene")),
        disabling_damage_tow_away=_clean(form.get("disabling_damage_tow_away")),
        hazmat_released_other_than_fuel=_clean(form.get("hazmat_released_other_than_fuel")),
        required_reports_attached=_clean(form.get("required_reports_attached")),
        loading_or_unloading_only=_clean(form.get("loading_or_unloading_only")),
        boarding_or_alighting_only=_clean(form.get("boarding_or_alighting_only")),
        dot_review_status=_clean(form.get("dot_review_status"), "not_indicated"),
        dot_review_note=_clean(form.get("dot_review_note")),
        driver_performing_safety_sensitive_function=_clean(form.get("driver_performing_safety_sensitive_function")),
        loss_of_human_life=_clean(form.get("loss_of_human_life")),
        moving_violation_citation=_clean(form.get("moving_violation_citation")),
        citation_time=_parse_datetime(form.get("citation_time")),
        bodily_injury_treatment_away_from_scene=_clean(form.get("bodily_injury_treatment_away_from_scene")),
        tow_away_disabling_damage=_clean(form.get("tow_away_disabling_damage")),
        ...
        manager_notified=_clean(form.get("manager_notified")) or manager_notified_quick,
        manager_notified_at=_parse_datetime(form.get("manager_notified_at")),
        dispatcher_notified=_clean(form.get("dispatcher_notified")),
        dispatcher_notified_at=_parse_datetime(form.get("dispatcher_notified_at")),
        replacement_vehicle_needed=_clean(form.get("replacement_vehicle_needed")),
        driver_relieved_from_route=_clean(form.get("driver_relieved_from_route")),
        photos_required_complete=_clean(form.get("photos_required_complete")),
